...
* Built-in metrics and observability (transaction counts, timing, errors)
...
[source,clojure]
----
{:deps {us.whitford/fulcro-rad-datalevin {:git/url "https://github.com/mwhitford/fulcro-rad-datalevin"
                                           :git/sha "LATEST_SHA"}}}
----
...
(def env
  (-> indexes
      (assoc ::dlo/connections @db/connections)))
----
...
`get-metrics`:: Get current database operation metrics
+
[source,clojure]
----
(dl/get-metrics)
...
;;     :query-count 100
;;     :total-transaction-time-ms 1234}
----

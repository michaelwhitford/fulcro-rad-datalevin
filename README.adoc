...
NOTE: This library does *not* bundle Pathom. Add the Pathom version your app
...
by RAD's Pathom 3 processor (see <<pathom-setup>>).
...
TIP: If you build a Pathom 3 index yourself (`pci/register`) instead of using
...
`generate-resolvers-pathom3`:: Like `generate-resolvers`, but returns native Pathom 3 resolver records. Use only when you build a Pathom 3 index directly (e.g. `pci/register`) rather than via `new-processor`. Pathom 3 is resolved lazily at call time, so it stays optional.
+
[source,clojure]
----
...
`::dlo/databases` snapshot into the env. See <<pathom-setup>> for full wiring.

`pathom-plugin`:: Create a *Pathom 2* plugin (a `:com.wsscode.pathom.core/wrap-parser` map) that injects `::dlo/connections` and `::dlo/databases` into the parse env. Pass a `(fn [env])` returning the schema -> connection map.
+
[source,clojure]
----
...
`wrap-env`:: The *Pathom 3* integration point: an `(fn [env] env')` that injects `::dlo/connections` and `::dlo/databases`. Compose it into the `env-middleware` you pass to RAD's `new-processor` (it also underlies the Pathom 2 `pathom-plugin`).
+
[source,clojure]
----
(-> (attr/wrap-env all-attributes)
...
=== Vector Attributes (`:vec`) and Similarity Search
...
| Required. The query embedding — a sequence of numbers matching the
  attribute's `:db.vec/dimensions`. Missing resolves to an empty list.

| `:attribute`
| Optional qualified keyword narrowing the search to one `:vec` attribute
  (e.g. `:account/embedding`). Defaults to all of the entity's vector domains
  (which must then share dimensions).

| `:top`
| Cap on neighbors returned (Datalevin default 10).
|===

Vector indexing is synchronous — saved entities are immediately searchable.
Native-id entities are supported (the matched eid is the id).
...
=== Full-Text Search (`::dlo/fulltext?`)

Mark any attribute searchable and the adapter wires the whole feature — schema,
connection configuration, and a generated, parameterized search resolver:
...
| Required. A string, a boolean expression vector such as
  `[:and "red" [:not "dog"]]`, or `{:phrase "quick red fox"}` (phrase search
  requires `{:index-position? true}` on the attribute). Missing/blank resolves
  to an empty list.
...
* Indexing is synchronous — saved entities are immediately searchable
  (read-your-writes).
...
Builds use https://github.com/clojure/tools.build[tools.build] (see `build.clj`).
...
2. **Missing database connection** - Verify the Pathom env has `::dlo/connections`. This is injected from the `(fn [env])` you provide to `dl/pathom-plugin` (Pathom 2) or `dl/wrap-env` (Pathom 3).
